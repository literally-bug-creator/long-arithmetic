...
TEST(Multiplication, Manual_LargePositiveExponents) {
    ...
    // Используем сравнение с относительной погрешностью для чисел с плавающей запятой
    mpf_class rel_err = abs(prod_bn - prod_gmp) / abs(prod_gmp);
    ASSERT_LE(rel_err.get_d(), 1e-15); // Проверяем, что относительная ошибка очень мала
}
