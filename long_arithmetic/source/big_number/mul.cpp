...
BigNumber simple_mul(const BigNumber& multiplicand,
    ...
    for (int32_t i = 0; i < get_size(multiplicand); ++i) {
        ...
        for (int32_t j = 0; j < get_size(multiplier); ++j) {
            mul_chunk l_chunk = static_cast<mul_chunk>(get_chunk_direct(multiplicand, i));
            mul_chunk r_chunk = static_cast<mul_chunk>(get_chunk_direct(multiplier, j));
    ...
    for (size_t len = 2; len <= n; len <<= 1) {
        ...
        for (size_t i = 0; i < n; i += len) {
            ...
            for (size_t j = 0; j < (len >> 1); ++j) {
                uint32_t u = a[i + j];
                uint32_t v = static_cast<uint64_t>(a[i + j + (len >> 1)]) * w % mod;
